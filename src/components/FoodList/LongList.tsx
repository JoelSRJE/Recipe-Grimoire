...
const LongList = ({
  ...
  const activeEditMode = (food: LongFood) => {
    ...
    <div className="flex flex-col gap-2 p-2">
      <div className="flex justify-between items-center">
        <h1 className="flex items-center gap-2 font-bold text-2xl">
          Long Foods <MoonIcon size={24} className="text-green-highlight" />
        </h1>
        <span className="text-sm text-darker-text">({longFoods.length}x)</span>
      </div>

      {/* The Long List */}
      {longFoods.length > 0 ? (
        <ul className="flex flex-col gap-2">
          {longFoods.map((food, idx) => {
